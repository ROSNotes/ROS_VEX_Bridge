...
"""
    A Python driver for the VEX microcontroller running the
    ROSVEXBridge firmware.
...
class VEX:
    ...
    def __init__(self, port="/dev/ttyUSB0", baudrate=57600, timeout=0.5, motors_reversed=False):
        ...
        self.PID_RATE = 30 # Do not change this!  It is a fixed property of the VEX PID controller.
    ...
    def connect(self):
        try:
            print "Connecting to VEX on port", self.port, "..."
    ...
    def recv(self, timeout=0.5):
        ...
        ''' This command should not be used on its own: it is called by the execute commands
            ...
            readline() tends to return garbage characters from the VEX
        '''
        c = ''
        value = ''
        attempts = 0
    ...
    def execute(self, cmd):
        ''' Thread safe execution of "cmd" on the VEX returning a single integer value.
        '''
        self.mutex.acquire()
    ...
    def execute_array(self, cmd):
        ''' Thread safe execution of "cmd" on the VEX returning an array.
        '''
        self.mutex.acquire()
    ...
    def execute_ack(self, cmd):
        ''' Thread safe execution of "cmd" on the VEX returning True if response is ACK.
        '''
        self.mutex.acquire()
    ...
    def update_pid(self, Kp, Kd, Ki, Ko):
        ''' Set the PID parameters on the VEX
        '''
...
if __name__ == "__main__":
    ...
    myVEX = VEX(port=portName, baudrate=baudRate, timeout=0.5)
    myVEX.connect()

    print "Sleeping for 1 second..."
    time.sleep(1)

    print "Reading on analog port 0", myVEX.analog_read(0)
    print "Reading on digital port 0", myVEX.digital_read(0)
    print "Blinking the LED 3 times"
    for i in range(3):
        myVEX.digital_write(13, 1)
        time.sleep(1.0)
    #print "Current encoder counts", myVEX.encoders()
